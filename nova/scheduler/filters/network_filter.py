...
class NetworkFilter(filters.BaseHostFilter):
    """ Filter the host pased on the net pci devices available
        ...
        capabilities. """
    def __init__(self):
        self.passthru_networks = FLAGS.pci_passthru_networks;
...
#        LOG.debug('##### NetworkHostsFilter for %s #####' % filter_properties);
        ...
        compute_node = db.compute_node_get_by_host(context, host_state.host)
        host_dev_list = json.loads(compute_node['net_pci_passthru']);
