...
class PciDriver(object):
    ...
    def __init__(self):
        ...
        self._inventory_managers = {
            ...
            'net_vf': NetPciInventoryManager('net_vf', self),
            'net': NetPciInventoryManager('net', self)}
    ...
    def get_allocated_pci_devices(self, instance):
        ...
        for pci_class_set in instance_alloc_devices_dict.itervalues():
            for pci_dev in pci_class_set:
                ...
                index = 0;
                for netid in network_id_list:
                    if pci_dev.network_id < netid: break;
                    else: index += 1;

                network_id_list.insert(index, pci_dev.network_id);
                out_list.insert(index, dict(pci_domain='0x' + pci_domain,
                                     pci_bus='0x' + pci_bus,
    ...
    def get_all_free_net_pci_devices(self, pci_class_list):

        netdev_list = list();
        all_alloc_devices_dict, ignored = \
            self._find_allocated_pci_devices(-1, False) # All devices;

        for pci_class in pci_class_list:
            ...
            for pci_dev in alloc_for_pci_class_set:
                network_bucket = \
                    inventory_mgr._network_buckets.get(pci_dev.network_id)
            ...
            LOG.debug('##### Available list for pci class: %s' % pci_class);
            for network_bucket in inventory_mgr._network_buckets.itervalues():
                dev = dict();
                dev['network_id'] = network_bucket.network_id;
                dev['pci_class'] = pci_class;
                dev['avail'] = 0;

                for parent_pf, pf_set in network_bucket._pf_sets_dict.iteritems():
                    dev['avail'] = dev['avail'] + len(pf_set);
                network_bucket.clear_allocated()
                dev['total'] = len(network_bucket._pf_sets_dict);
                netdev_list.append(dev)
                LOG.debug('##### netid: %s, pci_class: %s, avail=%d, total: %d' %
                (dev['network_id'], dev['pci_class'], dev['avail'], dev['total']))
    ...
    def _pci_device_allocation_required(self, instance_type):
        ...
        extra_specs = self.capi.instance_type_extra_specs_get(self._context,
    ...
    def _find_allocated_pci_devices(self, instance_id, do_print=True):
        """
        ...
        instances = self.capi.instance_get_all_by_host(self._context, self.host)
        if not instances: # THIS IS A HACK TO PASS run_tests
            instances = []
    ...
    def _commit_instance_pci_devices_allocation(self, instance_uuid,
                                                instance_alloc_devices_dict):
        """
        ...
        self.capi.instance_metadata_update(self._context, instance_uuid,
                                    metadata_upd_dict,
...
##############################################################################
class NetPciInventoryManager(PciInventoryManager):
    """
    Implementation of PciInventoryManager for the 'net/net_vf' PCI device class
    """
    def __init__(self, pci_class, pci_driver):
        if pci_class != 'net' and \
            pci_class != 'net_vf':
                raise exception.InvalidInput(reason=_("pci_class should be "\
                    "'net/net_vf', not %(pci_class)s") % locals())

        super(NetPciInventoryManager, self).__init__(pci_class, pci_driver)
        # key: network_id, value: NetPciNetworkBucket
    ...
    def add_pci_device(self, pci_dev):
        ...
        if self.pci_class == 'net_vf':
            ...
            if not hasattr(pci_dev, "network_id"):
                raise exception.InvalidInput(reason=_("Not all net-specific "\
                    "parameters specified for %(pci_dev)s") % locals())

            if hasattr(pci_dev, "parent_pf"):
                raise exception.InvalidInput(reason=_("Invalid net-specific "\
                    "parameters specified for %(pci_dev)s") % locals())

            # Full pci passthrough, parent is sys, since there is no eth dev associated with it.
            pci_dev.parent_pf = 'sys'
        ...
        if not network_bucket:
            network_bucket = \
                NetPciNetworkBucket(pci_dev.network_id)
    ...
    def normalize_allocation_requirements(self, instance_name,
        ...
        if len(normalized_alloc_requirement_dict) > 0:
            LOG.info(_("NetPciInventoryManager: Normalized PCI allocation "\
                        "requirements:"))
    ...
    def update_allocation_requirements(self, alloc_requirement_list,
        ...
        LOG.info(_("NetPciInventoryManager: updating allocation "\
                    "requirements"))
    ...
    def has_network_pci_devices(self, pci_class_set,
        ...
        """
        This method is specific to NetPciInventoryManager class.
...
##############################################################################
class NetPciNetworkBucket(object):
    """
    Represents an inventory of all 'net' PCI devices that have
    the same network_id
    """
    ...
    def add_pci_device(self, pci_dev, do_print=True):
	    ...
	    Also from clear_allocated after allocation.
        """
        ...
        if do_print:
            ...
            LOG.info(_("NetPciNetworkBucket %(self_id)s, "\
    ...
    def mark_pci_device_allocated(self, pci_dev):
        ...
        if not pf_set:
            ...
            LOG.warn(_("NetPciNetworkBucket %(self_id)s, "\
    ...
    def allocate_pci_devices(self, count):
        ...
        if CONF.pci_class_netvf_distribute_alloc_across_parentpf and \
            (count % num_pf_sets) != 0:
            LOG.error(_("NetPciNetworkBucket %(self_id)s, network_id "\
    ...
    def print_free_pci_devices(self):
        self_id = id(self)
        self_net_id = self.network_id
        LOG.info(_("NetPciNetworkBucket %(self_id)s, network_id "\
